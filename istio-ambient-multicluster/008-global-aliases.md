...
Remove the `solo.io/service-scope=global` labels that lab `007` added to the backend services. Lab `006`
scoped only `productpage` globally, so this returns the mesh to the baseline the later labs expect:
```bash
for context in $KUBECONTEXT_CLUSTER1 $KUBECONTEXT_CLUSTER2; do
