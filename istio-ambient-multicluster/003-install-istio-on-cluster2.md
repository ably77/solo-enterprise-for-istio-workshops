...
> ```bash
> kubectl apply -f gke/resourcequota.yaml --context $CLUSTER2
> ```
...
```bash
kubectl rollout status ds/istio-cni-node -n istio-system --watch --timeout=90s --context $CLUSTER2
```
...
Wait for rollout to complete
```bash
kubectl rollout status ds/ztunnel -n istio-system --watch --timeout=90s --context $CLUSTER2
```
