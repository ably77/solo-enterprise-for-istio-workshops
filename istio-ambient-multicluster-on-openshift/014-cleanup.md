...
On OpenShift, `ztunnel` and `istio-cni` install into `kube-system` rather than `istio-system`, so the
uninstall commands below target that namespace.
...
for CONTEXT in "${CLUSTERS[@]}"; do
  ...
  # Peering (lab 006 Option B)
  helm uninstall peering-remote -n istio-gateways --kube-context $CONTEXT --ignore-not-found

  # Istio
  helm uninstall ztunnel -n kube-system --kube-context $CONTEXT --ignore-not-found
  helm uninstall istiod -n istio-system --kube-context $CONTEXT --ignore-not-found
  helm uninstall istio-cni -n kube-system --kube-context $CONTEXT --ignore-not-found
...
SCC grants are cluster-scoped, so deleting the namespaces above leaves them behind. Lab `001` grants
`anyuid` to the two Bookinfo service account groups on both clusters:

```bash
for CONTEXT in "${CLUSTERS[@]}"; do
  oc --context $CONTEXT adm policy remove-scc-from-group anyuid system:serviceaccounts:bookinfo-frontends
  oc --context $CONTEXT adm policy remove-scc-from-group anyuid system:serviceaccounts:bookinfo-backends
done
```

If you installed the Solo Management UI in lab `013`, remove the ClickHouse grant from `cluster1` too:

```bash
oc --context $KUBECONTEXT_CLUSTER1 adm policy remove-scc-from-user anyuid -z management-clickhouse -n kagent
```
