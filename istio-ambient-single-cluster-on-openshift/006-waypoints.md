...
## Prerequisites
- This lab assumes you have completed labs `000`–`004`
...
ztunnel handles Layer 4 authorization — policies that allow or deny based on workload identity (SPIFFE). Because ztunnel operates at L4, it cannot inspect HTTP attributes such as method or path.
...
In the next step `007` we will inspect mesh metrics using the Istio observability endpoints.
